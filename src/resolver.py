"""
Deterministic resolution for the CCTV query agent: camera names AND
date/time expressions.

This module is where all non-LLM, rule-based interpretation lives, per
the architecture split in README.md: the LLM extracts *what the user
said*, and resolver.py turns that into *what it actually means* —
canonical camera names and absolute date/time boundaries.

Camera resolution:
    "CTE" / "Central Expressway" / "central expressway"
        -> "Central Expressway (CTE)"
    "Kranji Highway" -> "Kranji Expressway (KJE)"
    "Tampines Expresway" (typo) -> "Tampines Expressway (TPE)"

Date/time resolution:
    "today", "yesterday", "this week", "last month", ... -> absolute,
    timezone-aware datetime boundaries.
    "15th to 18th of last month" -> a concrete date range.
    "8 AM to 10 AM" -> a daily time-of-day filter.
    "every Tuesday" -> a SQLite strftime('%w') weekday code.

Neither half of this module touches the database or generates SQL —
that is query_builder.py's job, working from the ResolvedQuery that
query_schema.py builds using the functions here.
"""
...
from config.metadata import CAMERA_ALIASES, CAMERAS
...
def resolve_camera(user_input: str) -> str | None:
    ...
    # 2. Fuzzy match (typos, minor wording differences)
...
if __name__ == "__main__":
    print(resolve_camera("Kranji Highway"))
    print(resolve_camera("Tampines Expresway"))
    print(resolve_camera("Mars Expressway"))
